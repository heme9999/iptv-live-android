...
- AndroidX Media3 / ExoPlayer 播放 HLS、FLV 等直播流，支持跨协议重定向与动态 HLS 嗅探
- 官方源与全球 CDN 实时同步，支持 App 内「🔄 一键刷新播放清单」无需升级 APK
- 支持自定义手工添加 / 更换任意 M3U 订阅源，并支持一键恢复官方内置源
- TCL / Sony 电视遥控器焦点导航，Pixel 手机触控布局
- 主页、直播、频道分类、设置四个一级入口
- 可选择启动页面、自动播放行为，并记忆最近观看频道
- Pixel 选择频道后自动隐藏菜单进入全屏，返回键恢复菜单
- Pixel 全屏播放时轻触画面可从左侧弹出频道菜单并直接切台
- 仅 Pixel 手机版支持系统画中画（PiP），关闭小窗时硬件级 0 毫秒静音切断
- 频道条目异步显示连接延时或超时状态
- TCL/Sony 选择频道后自动全屏，确定、菜单或返回键恢复频道菜单
- Pixel 使用 APTV 风格大厅：双列频道卡片、筛选入口、播放状态卡和下方频道列表
